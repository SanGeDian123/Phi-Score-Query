...
async fn seed_leaderboard_db(storage: &StatsStorage) {
    ...
    for (user_hash, alias, score) in users {
        ...
        let nickname = format!("{alias} Game");
        storage
...
async fn leaderboard_top_masks_next_after_user_and_supports_lite() {
    ...
    assert_eq!(items[0]["nickname"], "Alice Game");
    assert_eq!(items[0]["avatar"], "Glaciaxion");
    assert_eq!(items[0]["challengeModeRank"], 503);
