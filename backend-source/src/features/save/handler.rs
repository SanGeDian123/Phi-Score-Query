...
pub(crate) fn official_avatar_name(value: &str) -> Option<String> {
    static OFFICIAL_AVATARS: OnceCell<HashSet<&'static str>> = OnceCell::new();
    let avatars = OFFICIAL_AVATARS.get_or_init(|| {
    ...
    avatars.contains(value).then(|| value.to_string())
}
...
fn spawn_leaderboard_write(
    ...
    tokio::spawn(async move {
        ...
        if let Err(e) = storage
            ...
            .await
        {
            tracing::warn!(target: "phi_backend::leaderboard", user_hash = %user_hash, "update player profile failed (ignored): {e}");
        }
    });
}
...
pub async fn get_save_data(
    ...
    let player_nickname = if let Some(session_token) = auth.payload.session_token.as_deref() {
    ...
    // user.dat 中的头像字段是完整官方名称；summary 中的短字段在部分存档会被解析成
    // "..." 或控制字符，因此仅在命中官方头像表时才允许写入排行榜。
    let player_avatar = data
        .parsed
        .user
        .as_ref()
        .and_then(|user| official_avatar_name(&user.avatar))
        .or_else(|| {
            data.parsed
                .summary_parsed
                .as_ref()
                .and_then(|summary| official_avatar_name(&summary.avatar))
        });
    let challenge_mode_rank = data
        .parsed
        .summary_parsed
        .as_ref()
    ...
    let response = if let Some(ref rks_result) = rks_opt {
        if calc_rks {
            ...
            build_save_response(&data, None, player_nickname.clone())?
        }
    } else {
        build_save_response(&data, None, player_nickname.clone())?
    };
...
#[cfg(test)]
mod tests {
    use super::{build_save_cache_key, official_avatar_name};
    ...
    #[test]
    fn avatar_name_accepts_official_user_value_and_rejects_broken_summary_value() {
        assert_eq!(
            official_avatar_name("Glaciaxion").as_deref(),
            Some("Glaciaxion")
        );
        assert!(official_avatar_name("...").is_none());
        assert!(official_avatar_name("\u{e}").is_none());
    }
}
