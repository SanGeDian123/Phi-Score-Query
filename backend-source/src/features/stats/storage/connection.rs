...
impl StatsStorage {
    ...
    pub async fn init_schema(&self) -> Result<(), AppError> {
        ...
        self.ensure_user_profile_player_columns().await?;
        Ok(())
    }
    ...
    /// 为旧排行榜数据库补齐玩家展示资料列；已存在时跳过。
    async fn ensure_user_profile_player_columns(&self) -> Result<(), AppError> {
        let rows = sqlx::query("PRAGMA table_info(user_profile)")
            .fetch_all(&self.pool)
            .await
            .map_err(|e| AppError::Internal(format!("table_info user_profile: {e}")))?;
        ...
        for (name, sql_type) in [
            ...
            if !columns.contains(name) {
                sqlx::query(&format!(
                    "ALTER TABLE user_profile ADD COLUMN {name} {sql_type}"
                ))
                .execute(&self.pool)
                .await
                .map_err(|e| AppError::Internal(format!("alter user_profile {name}: {e}")))?;
            }
        }
        Ok(())
    }
