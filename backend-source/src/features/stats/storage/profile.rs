...
impl StatsStorage {
    pub async fn update_player_profile(
        ...
        sqlx::query(
            "UPDATE user_profile SET nickname=COALESCE(?,nickname), avatar=COALESCE(?,avatar), challenge_mode_rank=COALESCE(?,challenge_mode_rank), updated_at=? WHERE user_hash=?",
        )
        .bind(nickname)
        .bind(avatar)
        .bind(challenge_mode_rank)
        .bind(now_rfc3339)
        .bind(user_hash)
        .execute(&self.pool)
        .await
        .map_err(|e| AppError::Internal(format!("update player profile: {e}")))?;
        Ok(())
    }
