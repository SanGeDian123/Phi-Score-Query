...
fn official_avatar_name(value: String) -> Option<String> {
    static OFFICIAL_AVATARS: OnceLock<HashSet<&'static str>> = OnceLock::new();
...
async fn build_leaderboard_items(
    ...
    for (idx, r) in rows.into_iter().enumerate() {
        ...
        let avatar: Option<String> = r.try_get("avatar").ok().and_then(official_avatar_name);
        let challenge_mode_rank: Option<i64> = r.try_get("challenge_mode_rank").ok();
