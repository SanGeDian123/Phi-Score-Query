...
pub(super) struct BnTemplateLayout {
    ...
    pub section_heading_height: u32,
}
...
struct HeaderCtx {
    title_xml: String,
    player_name_xml: String,
    real_rks_text_xml: String,
    data_text_xml: String,
    challenge_text_xml: String,
    updated_text_xml: String,
    avatar_href_xml: Option<String>,
...
fn challenge_mode_text(challenge_rank: Option<&(String, String)>) -> String {
    let Some((color, level)) = challenge_rank else {
        return "Challenge Mode --".to_string();
    };
    let color_short = match color.as_str() {
        "Green" => "绿",
        "Blue" => "蓝",
        "Red" => "红",
        "Gold" => "黄",
    ...
    format!("Challenge Mode {color_short}{level}")
}
    ...
    let avatar_dir = std::env::var_os("APP_AVATAR_DIR")
        ...
        .or_else(|| {
            std::env::current_exe()
                ...
                .parent()?
                .parent()
                .map(|install_root| install_root.join("avatar"))
        })?;
    let hash = hex::encode(Sha256::digest(avatar_name.as_bytes()));
    let path = avatar_dir.join(format!("{hash}.png"));
    get_scaled_image_data_uri(&path, 360, 200)
}
    ...
    S: std::hash::BuildHasher,
{
    ...
    let ap_card_start_y = card_gap + layout.section_heading_height;
    ...
    let mut next_y = main_content_start_y + card_gap + layout.section_heading_height;
    ...
    let ctx = BnTemplateCtx {
        ...
        header: HeaderCtx {
            ...
            player_name_xml: escape_xml(
                stats
                    .player_name
                    .as_deref()
                    .unwrap_or(super::DEFAULT_PLAYER_NAME),
            ),
            real_rks_text_xml: escape_xml(&format!("RKS {:.4}", stats.real_rks.unwrap_or(0.0))),
            data_text_xml: escape_xml(stats.data_string.as_deref().unwrap_or("Data --")),
            challenge_text_xml: escape_xml(&challenge_mode_text(stats.challenge_rank.as_ref())),
            updated_text_xml: escape_xml(&format!(
                "更新时间 {}",
                stats.update_time.format("%Y/%m/%d %H:%M:%S UTC")
            )),
            avatar_href_xml: player_avatar_data_uri(stats.player_avatar.as_deref())
        ...
        main_section_y: main_content_start_y,
        cards,
        footer: FooterCtx {
            y: footer_y,
