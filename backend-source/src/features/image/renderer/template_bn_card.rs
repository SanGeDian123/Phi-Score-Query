...
struct CardTextCtx {
    ...
    song_name_lines_xml: Vec<String>,
    score_text: String,
    acc_text_xml: String,
    ...
    S: std::hash::BuildHasher,
{
    ...
    let cover_h = ctx
        .layout
        .cover_height
        .unwrap_or(text_block_height)
        .max(1.0);
    let cover_w = cover_h * COVER_ASPECT_RATIO;
    let card_h =
        round_non_negative_to_u32((cover_h.max(text_block_height) + card_padding * 2.0).max(1.0));
    ...
    CardCtx {
        ...
        text: CardTextCtx {
            ...
            song_name_lines_xml: song_lines.iter().map(|line| escape_xml(line)).collect(),
            score_text,
