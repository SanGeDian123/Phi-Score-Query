...
pub(super) fn image_footer_text(app_version: Option<&str>) -> Option<String> {
    normalized_app_version(app_version)
        .map(|version| format!("Phi Score Query · {version}"))
        .or_else(|| Some(AppConfig::global().branding.footer_text.clone()))
}

pub(super) fn normalized_app_version(app_version: Option<&str>) -> Option<String> {
