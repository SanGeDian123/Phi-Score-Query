...
#[test]
fn image_footer_text_uses_valid_app_version_and_rejects_unsafe_values() {
    let _ = crate::config::AppConfig::init_global();

    assert_eq!(
        super::normalized_app_version(Some(" Pre-0.9.6.9-Fix ")),
        Some("Pre-0.9.6.9-Fix".to_string())
    );
    assert_eq!(
        super::image_footer_text(Some("Pre-0.9.6.9-Fix")),
        Some("Phi Score Query · Pre-0.9.6.9-Fix".to_string())
    );
    assert_eq!(super::normalized_app_version(Some("<script>")), None);
    assert_eq!(
        super::image_footer_text(Some("<script>")),
        Some(
            crate::config::AppConfig::global()
                .branding
                .footer_text
...
#[test]
fn bn_cache_key_is_isolated_by_app_version() {
    let _ = crate::config::AppConfig::init_global();
