...
$releaseNotesPath = Join-Path $bundleRoot 'changelog-Pre-0.9.7.7.json'
$apkName = 'Phi-Score-Query-Pre-0.9.7.7.apk'
...
$backupRoot = Join-Path $installRoot ('backup\pre-0.9.7.7-' + (Get-Date -Format 'yyyyMMdd-HHmmss'))
$backupUpdateRoot = Join-Path $backupRoot 'app-update'
New-Item -ItemType Directory -Force -Path $backupRoot, $backupUpdateRoot, $appUpdateRoot, $announcementRoot | Out-Null
...
try {
    ...
    New-Item -ItemType Directory -Force -Path (Split-Path $fontTarget -Parent), (Split-Path $assetsTarget -Parent), (Split-Path $publishUpdateTarget -Parent) | Out-Null
    ...
    if ([int] $latest.versionCode -ne 35 -or [string] $latest.versionName -ne 'Pre-0.9.7.7') {
    ...
    if (-not (Test-Path -LiteralPath $fontTarget) -or -not (Test-Path -LiteralPath $assetsTarget)) {
        throw 'Phi-Plugin resources were not installed.'
    }

    Write-Output 'Pre-0.9.7.7 backend, Phi-Plugin resources, APP update, and announcement support deployed.'
    ...
    throw "Pre-0.9.7.7 deployment failed and was rolled back. Original error: $($_.Exception.Message)"
}
