...
$releaseNotesPath = Join-Path $bundleRoot 'changelog-Pre-0.9.7.8-Fix.json'
$apkName = 'Phi-Score-Query-Pre-0.9.7.8-Fix.apk'
...
if ($ValidateOnly) {
    Write-Output 'Pre-0.9.7.8-Fix package hashes, release metadata, and Caddy configuration are valid.'
    return
}

$backupRoot = Join-Path $installRoot ('backup\pre-0.9.7.8-fix-' + (Get-Date -Format 'yyyyMMdd-HHmmss'))
...
try {
    ...
    if ([int] $latest.versionCode -ne 40 -or [string] $latest.versionName -ne 'Pre-0.9.7.8-Fix') {
    ...
    Write-Output 'Pre-0.9.7.8-Fix backend, suggestion API, Phi-Plugin renderer, and APP update deployed.'
    ...
    $failure = "Pre-0.9.7.8-Fix deployment failed during '$failedStep'. Original error [$originalType]: $originalMessage"
