...
$releaseNotesPath = Join-Path $bundleRoot 'changelog-Pre-0.9.7.2.json'
$apkName = 'Phi-Score-Query-Pre-0.9.7.2.apk'
...
$backupRoot = Join-Path $installRoot ('backup\app-update-pre-0.9.7.2-' + (Get-Date -Format 'yyyyMMdd-HHmmss'))
...
try {
    ...
    if ([int] $latest.versionCode -ne 28 -or [string] $latest.versionName -ne 'Pre-0.9.7.2') {
    ...
    Write-Output 'Pre-0.9.7.2 app update published.'
    ...
    throw "Pre-0.9.7.2 app update failed and was rolled back. Original error: $($_.Exception.Message)"
}
