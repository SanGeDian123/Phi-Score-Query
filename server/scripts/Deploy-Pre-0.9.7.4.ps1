...
$releaseNotesPath = Join-Path $bundleRoot 'changelog-Pre-0.9.7.4.json'
$apkName = 'Phi-Score-Query-Pre-0.9.7.4.apk'
...
$backupRoot = Join-Path $installRoot ('backup\app-update-pre-0.9.7.4-' + (Get-Date -Format 'yyyyMMdd-HHmmss'))
...
try {
    ...
    if ([int] $latest.versionCode -ne 32 -or [string] $latest.versionName -ne 'Pre-0.9.7.4') {
    ...
    Write-Output 'Pre-0.9.7.4 app update published.'
    ...
    throw "Pre-0.9.7.4 app update failed and was rolled back. Original error: $($_.Exception.Message)"
}
