...
$apkPath = Join-Path $bundleRoot 'Phi-Score-Query-Pre-0.9.6.5.apk'
$appUpdateRoot = Join-Path $InstallRoot 'app-update'
$latestTarget = Join-Path $appUpdateRoot 'latest.json'
$publishedApkTarget = Join-Path $appUpdateRoot 'Phi-Score-Query-Pre-0.9.6.5.apk'
$expectedApkHash = '179e333871b993ea0fce4a4f3db35c9566601123e92d581259fb405055aeaacb'
...
$backup = Join-Path $InstallRoot ("backup\pre-0.9.6.5-" + (Get-Date -Format 'yyyyMMdd-HHmmss'))
...
if ($hadPublishedApk) {
    Copy-Item -LiteralPath $publishedApkTarget -Destination (Join-Path $backup 'Phi-Score-Query-Pre-0.9.6.5.apk') -Force
}
...
try {
    ...
    if ([int] $latest.versionCode -ne 18 -or [string] $latest.versionName -ne 'Pre-0.9.6.5') {
    ...
    Write-Output "Pre-0.9.6.5 已发布。备份位于: $backup"
    ...
    if ($hadPublishedApk) {
        Copy-Item -LiteralPath (Join-Path $backup 'Phi-Score-Query-Pre-0.9.6.5.apk') -Destination $publishedApkTarget -Force
    }
